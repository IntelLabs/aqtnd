...
def random_1d_heis_cos(L, Jvar, Jmean, hvar, hmean, cos_k, S):
    ...
    jvals = np.random.normal(loc = Jmean, scale = np.sqrt(Jvar), size = L - 1) * np.cos(cos_k * jrange/ float(L))
    hvals = np.random.normal(loc = hmean, scale = np.sqrt(hvar), size = L)
...
for run_id in range(runs):    
	...
	H, j, h = random_1d_heis_cos(L, Jvar, Jm, hvar, hm, np.random.randint(cos_low, cos_high), S)
	tdn = np.zeros((L,2))
	tdn[:-1, 0] = j
	tdn[:,1] = h
